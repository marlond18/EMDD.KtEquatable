...
- Changes on the Attribute usage particularly `FloatingPointEquality` and `DoubleEnumerableEquality`
...
## Diagnostic Reports (added on 2.0.3)
### Compilation Error will occur when:
- static class is marked as [Equatable]
- [DoubleEnumerableEquality] and [FloatingPointEquality] precision attribute is less than zero.
- [ReferenceEquality] is used on a non-class property
- a non-class, non-record type declaration is marked as [Equatable], ex. a struct (will soon be added though)
- duplicate [Equatable]
### Compilation Warning will occur when:
- the class (excluding records ofcourse) is already implements IEquatable<T>
- the class/record derives from a base not implementing IEquatable<T>